...
-- 1. Write a q-uery to display 
-- customer full name with their title (Mr/Ms), 
-- both first name and last name are in upper case, 
-- customer email id, 
-- customer creation date and 
-- display customer’s category 
...
end as New_Price from Product p
where p.product_id NOT IN (select oi.Product_ID from ORDER_ITEMS oi) ORDER BY 
Inventory_Value Desc;

-- 3. Write a query to display 
-- Product_class_code, 
-- Product_class_description, 
-- Count of Product type in each productclass, 
...
INNER JOIN product p on pc.product_class_code=p.product_class_code 
group by pc.product_class_code,pc.product_class_desc
having sum(p.product_quantity_avail*p.product_price) >100000 ORDER BY inventory_value 
DESC;
...
-- Shipper name, 
-- City to which it is catering, 
-- num of customer catered by the shipper in the city and 
-- number of consignments delivered to that city for Shipper DHL 
-- [NOTE: TABLES to be used - SHIPPER,ONLINE_CUSTOMER, ADDRESSS, ORDER_HEADER]

-- SOLUTION:

select sp.Shipper_name, a.city, count(Distinct(oc.customer_id)) as No_Customer_Catered,
count((oc.customer_id)) as No_Consignments_Catered 
from Shipper sp inner join ORDER_Header oh
ON oh.shipper_ID = sp.shipper_ID inner join online_customer oc
ON oc.customer_id= oh.customer_id inner join address a
ON oc.address_id= a.address_id where sp.shipper_name in('DHL') group by a.city
order by sp.Shipper_name;

-- 6. Write a query to display 
-- product_id, 
-- product_desc, 
-- product_quantity_avail, 
-- quantity sold and 
...
SELECT p.product_id, p.product_desc, p.product_quantity_avail, count(oi.PRODUCT_QUANTITY) as QTY_SOLD, 
case when (p.PRODUCT_QUANTITY_AVAIL / count(oi.PRODUCT_QUANTITY)) < 0.1 then 'Low inventory, need to add inventory'
when (p.PRODUCT_QUANTITY_AVAIL / count(oi.PRODUCT_QUANTITY)) < 0.5 then 'Medium inventory, need to add some inventory'
when count(oi.PRODUCT_QUANTITY) is null then 'No Sales in past, give discount to reduce inventory'
...
SELECT p.product_id, p.product_desc, p.product_quantity_avail, count(oi.PRODUCT_QUANTITY) as QTY_SOLD, 
case when (p.PRODUCT_QUANTITY_AVAIL / count(oi.PRODUCT_QUANTITY)) < 0.2 then 'Low inventory, need to add inventory'
when (p.PRODUCT_QUANTITY_AVAIL / count(oi.PRODUCT_QUANTITY)) < 0.6 then 'Medium inventory, need to add some inventory'
when count(oi.PRODUCT_QUANTITY) is null then 'No Sales in past, give discount to reduce inventory'
else 'Sufficient inventory' 
end as Inventory_Status
from product_class pc inner join product p on p.PRODUCT_CLASS_CODE = pc.PRODUCT_CLASS_CODE
...
SELECT p.product_id, p.product_desc, p.product_quantity_avail, count(oi.PRODUCT_QUANTITY) as QTY_SOLD, 
case when (p.PRODUCT_QUANTITY_AVAIL / count(oi.PRODUCT_QUANTITY)) < 0.3 then 'Low inventory, need to add inventory'
when (p.PRODUCT_QUANTITY_AVAIL / count(oi.PRODUCT_QUANTITY)) < 0.7 then 'Medium inventory, need to add some inventory'
when count(oi.PRODUCT_QUANTITY) is null then 'No Sales in past, give discount to reduce inventory'
else 'Sufficient inventory' 
end as Inventory_Status
from product_class pc inner join product p on p.PRODUCT_CLASS_CODE = pc.PRODUCT_CLASS_CODE
...
select order_id, volume from (SELECT oi.order_id, (len*width*height) as Volume
FROM Order_Items oi INNER JOIN Product p ON oi.product_id = p.product_id
group by oi.order_id)tab where Volume <= (Select (len * width * height) AS carton_vol
FROM Carton WHERE carton_id=10) order by volume desc
Limit 1;
...
select o.customer_id, concat(upper(o.customer_fname),' ',upper(o.customer_lname)) Customer_Full_Name, sum(oi.product_quantity),
sum(oi.product_quantity*p.product_price) as Total_Value from online_customer o inner join order_header oh
on o.CUSTOMER_ID=oh.CUSTOMER_ID inner join order_items oi
on oh.ORDER_ID=oi.ORDER_ID inner join product p
on oi.PRODUCT_ID=p.PRODUCT_ID where oh.PAYMENT_MODE="Cash" and oh.ORDER_STATUS="Shipped" 
and (o.CUSTOMER_LNAME) like 'G%'
group by o.CUSTOMER_ID, Customer_Full_Name;

-- 9. Write a query to display 
-- product_id, 
-- product_desc and 
-- total quantity of products which are sold together with product id 201 and 
-- are not shipped to city Bangalore and New Delhi. 
...
FROM(SELECT oi.order_id, p.product_id, p.product_desc, SUM(product_quantity) AS Total_Quantity
FROM Order_Items oi INNER JOIN Product p
where oi.product_id = p.product_id
AND order_id IN (SELECT order_id FROM Order_Items
WHERE product_id = 201) AND p.product_id != 201
...
-- 10. Write a query to display the 
-- order_id,
-- customer_id and 
-- customer fullname, 
...
SELECT oh.order_id, oc.customer_id, CONCAT(oc.customer_fname, ' ', oc.customer_lname) AS 
Customer_Full_Name, SUM(oi.product_quantity) AS Total_Quantity
...
inner join address a on a.address_id = oc.address_id
where mod(oh.order_id,2) = 0 AND oh.order_status = 'Shipped' 
AND a.pincode NOT LIKE "5%"
group by oh.order_id, oc.customer_id, Customer_Full_Name;